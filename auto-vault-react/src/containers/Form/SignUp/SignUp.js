			...
			<div className={[classes["container"],"card"].join(" ")}>
				...
				<div className="card-body mx-auto" style={{maxWidth: "400px"}}>
					{ this.state.showModal ?
						<Modal show={this.state.showModal} onHide={this.onModalClose} centered>
						<Modal.Body style={{textAlign: "center"}}><span className={classes["error"]}>{this.props.err}</span></Modal.Body>
						<Modal.Footer>
					...
					<div className="form-group input-group">
						...
						<select value={this.state.consumerType} className="form-control" required onChange={this.onConsumerTypeChangeHandler}>
							<option disabled value="">Select Consumer Type</option>
					...
					<div className="form-group input-group">
						...
						<input required value={this.state.password} onChange={this.onPasswordChangeHandler} className="form-control" placeholder="Create password" type="password"/>
					</div> 
					<div className="form-group input-group">
						<div className="input-group-prepend">
							<span className="input-group-text"> <i className="fa fa-lock"></i> </span>
						</div>
						<input required className="form-control"  value={this.state.confirmPassword} onChange={this.onConfirmPasswordChangeHandler} placeholder="Repeat password" type="password"/>
					</div>                                      
...
const mapDispatchToProps = dispatch => {
	return {
		signUp: ( name, username, email, password, confirmPassword, phone, consumerType ) => dispatch( actions.signUp( name, username, email, password, confirmPassword, phone, consumerType ) ),
